...
def encontrar_porta_arduino():
    """Tenta encontrar a porta série à qual um Arduino pode estar ligado."""
    ports = serial.tools.list_ports.comports()
    for port in ports:
        if "Arduino" in port.description or \
           "CH340" in port.description or \
           "USB-SERIAL" in port.description.upper() or \
           "VID:PID=2341:0043" in port.hwid.upper() or \
           "VID:PID=1A86:7523" in port.hwid.upper():
            print(f"Porta Arduino encontrada: {port.device} ({port.description})")
            return port.device
    print("Nenhuma porta Arduino encontrada automaticamente. Por favor, define PORTA_SERIAL manualmente.")
    return None

# Função para calcular CRC-8 (deve corresponder à lógica do Arduino)
def calcular_crc8_py(data_bytes: bytes) -> int:
    """Calcula CRC-8 para uma sequência de bytes. Polinómio 0x07 (x^8 + x^2 + x + 1)."""
    crc = 0x00
    polinomio = 0x07 
...
# Funções para introduzir erros
def introduzir_erro_bit_unico(data_bytes: bytes, posicao_bit: int = -1) -> bytearray:
    """Inverte um único bit nos dados. Se posicao_bit=-1, escolhe aleatoriamente."""
    ...
    bit_idx_no_byte = posicao_bit_erro % 8 # 0 é LSB, 7 é MSB
...
def introduzir_erro_rajada(data_bytes: bytes, inicio_bit: int = -1, comprimento_rajada: int = 3) -> bytearray:
    """Inverte uma rajada de bits (sequência de bits consecutivos)."""
    dados_corrompidos = bytearray(data_bytes)
    total_bits = len(dados_corrompidos) * 8
...
if __name__ == "__main__":
    if PORTA_SERIAL is None:
        PORTA_SERIAL = encontrar_porta_arduino()
    
    if PORTA_SERIAL is None:
        print("Script terminado. Nenhuma porta Arduino encontrada ou definida.")
        exit()
    ...
    try:
        with serial.Serial(PORTA_SERIAL, BAUD_RATE, timeout=TIMEOUT_LEITURA) as arduino_serial:
            ...
            time.sleep(2.5) # Ajusta conforme o delay no setup do Arduino
            ...
            while True:
                try:
                    ...
                    # Verifica primeiro as mensagens de controlo
